...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen flex items-center justify-center bg-slate-50" }, _attrs))}><div class="w-full max-w-md bg-white p-8 rounded-3xl shadow-lg"><h1 class="text-2xl font-bold mb-6">Entrar</h1><form class="space-y-4"><div><label class="block text-sm font-medium mb-2">Email</label><input${ssrRenderAttr("value", email.value)} type="email" required class="w-full rounded-xl border border-slate-300 px-4 py-3"></div><div><label class="block text-sm font-medium mb-2">Senha</label><input${ssrRenderAttr("value", password.value)} type="password" required class="w-full rounded-xl border border-slate-300 px-4 py-3"></div><button type="submit" class="w-full rounded-2xl bg-slate-900 text-white px-4 py-3">Entrar</button></form><p class="text-sm text-slate-500 mt-4">Novo por aqui? `);
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`Crie uma conta`);
          } else {
            return [
              createTextVNode("Crie uma conta")
            ];
          }
        }),
        _: 1
...
//# sourceMappingURL=login-YXRtvTTP.mjs.map
